...
That installs the `akapulu` skill and the [docs MCP](https://docs.akapulu.com/mcp). The agent sends you to https://akapulu.com/api-keys to create a key, then sets `AKAPULU_API_KEY` in this project the way your app already loads env vars — not by asking you to paste the key into chat. After that it creates a Clara hosted link you can try, then shares docs for a custom UI and the rest of the product.
